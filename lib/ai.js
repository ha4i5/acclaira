...
export async function generate(userId, prompt, maxTokens = 1200) {
  const anthropic = getSetting(userId, "claude_api_key").value;
  const gemini = getSetting(userId, "gemini_api_key").value;
  if (anthropic) {
  ...
  throw new Error("No AI key configured. Add a Claude or Gemini API key in Settings → AI engines.");
}
